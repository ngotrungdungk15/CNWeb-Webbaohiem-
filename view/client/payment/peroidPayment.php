          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Thanh toán định kỳ</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                  ...
                  <tbody class="text-center">
                    <?php
                        ...
                        if($j_son!=null){
                            foreach($j_son as $row){
                                if($row['Status']==1){
                                  ...
                                  if($customers['CustomerID'] == $customer['CustomerID']){
                                    array_push($contractArray,array('contractID'=> $row['ContractID'], 'contractcode'=> $row['ContractCode'], 'FileID'=> $row['FileID']));
                                  }
                                }
                          ...
                          if($PPj_son!=null){
                               foreach($contractArray as $r){
                                foreach($PPj_son as $ro){
                                  $sothang = date('Y-m', strtotime($ro['NextStageDate']));
                                  if($ro['ContractID']== $r['contractID'] && $sothang == date("Y-m") && $ro['Status']== 0){
                                    ?>
                                    <tr>
                                        <form action="" method="POST">
                                            ...
                                            <td>
                                                ...
                                                <input type="hidden" name="FileID" value="<?php echo $r['FileID']; ?>">
                                            </td>
                                            <td>
                                                <?php 
                                                    echo $ro['Stage']; 
...
<?php
    if(isset($_POST['redirect'])){
       ...
       // $peroidicPaymentID= NULL;
        $transactionInfo= array('customerID'=>$customer['CustomerID'], 'contractID'=>$_POST['contractID'], 'peroidicPaymentID'=>$_POST['PerodicPaymentID'], 'transactionCode'=>$order_id, 'contractCode'=>$_POST['contractCode'], 'transactionValue'=>$_POST['Fee'], 'transactionDate'=>date('Y-m-d'), 'fileID'=> $_POST['FileID']);
        ...
        if($Tran['status']==1){
          //VNpay
            ...
            $vnp_OrderInfo = 'Thanh toán định kỳ';//$_POST['order_desc'];
            $vnp_OrderType = 'billpayment';
            $vnp_Amount = $_POST['Fee'] * 100;//$_POST['amount'] * 100;
            $vnp_Locale = 'vn';
            $vnp_BankCode = '';
